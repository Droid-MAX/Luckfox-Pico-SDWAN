...
usage(){
    echo "Usage: $0 {enable|disable} [--inbond <interface>] [--outbond <interface>]"
    echo "Options:"
    echo "  enable/disable      Enable or disable forwarding (required)"
    echo "  --inbond <iface>    Specify inbound interface (default: usb0)"
    echo "  --outbond <iface>   Specify outbound interface (default: eth0)"
    exit 1
}
